...
export abstract class System {
  ...
  protected getRandomRune(selectFrom: IRune[] = this.runes): Rune {
    /**
    ...
    return new Rune(selectFrom[randomNumber - 1]);
  }
  ...
  protected getRandomRunes(
    ...
    let mutableRuneSelectionList: Array<Rune> = JSON.parse(
      JSON.stringify(selectFrom),
    );
    const selectedRunes: Array<Rune> = [];

    for (let i = 0; i < count; i++) {
      const rune = this.getRandomRune(mutableRuneSelectionList);
      // Remove the randomly chosen rune from the list of selections
      mutableRuneSelectionList = mutableRuneSelectionList.filter((r: Rune) => {
